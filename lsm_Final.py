...
def main():
	...
	vm_rec_nrn_popul = SimObjectPopulation(net, random.sample( all_nrn_popul.idVector(), nRecordNeurons ));# ah, i see. This is a sample of the main liquid. gets the ids for how many should be recorded from(nRecordNeurons). 
	...
	# Create an empty python list where we will going to store the output membrane voltage levels
	# for different runs of the experiment
	voltageLevels = []
	
	for i in range(vm_recorders_popul.size() ):
		# Retrieve the list of analog ?levels from the voltageMembrane recorder
    		analogLevel = list(net.object(vm_recorders_popul[i]).getRecordedValues())
    		# Store the membrane voltage levels in the list
    		voltageLevels.append(analogLevel)
		analogLevel = []
	
	
	#print len(analogLevel)
	#print len(voltageLevels)
	#print voltageLevels

	## plot the membrane potential
	#figure(1)
	#plot(arange(0,1,1e-5), voltageLevels[0]) #http://docs.scipy.org/doc/numpy/reference/generated/numpy.arange.html
	#xlim(0,1)   
	#title('Membrane Potential')
	
	#show()

	writeToTextFile(voltageLevels)
